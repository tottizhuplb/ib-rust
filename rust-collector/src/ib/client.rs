...
use crate::domain::{now_ns, ConnectionEvent, MarketEvent};
use crate::pipeline::{EventPublisher, MarketDataSource, SubscriptionControl, SubscriptionId};

/// IB Gateway client wrapper. Subscription streams will be bridged into [`MarketEvent`].
...
impl MarketDataSource for IbGatewayClient {
    async fn connect(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(client) = self.client.as_ref() {
            ...
            let _ = self
                .publisher
                .publish(MarketEvent::Control(crate::domain::ControlEvent {
                    ts_ns: now_ns(),
...
impl SubscriptionControl for IbGatewayClient {
    async fn subscribe_top(&self, symbol: crate::domain::Symbol) -> anyhow::Result<SubscriptionId> {
        let _client = self.client.as_ref().context("IB client not connected")?;
        let _contract = crate::ib::contract::equity_contract(&symbol);

        // TODO: ibapi v3 market data subscription -> bridge into TopOfBookEvent
