...
use crate::core::domain::{DesiredSubscription, SubscriptionEntry};

use super::model::{resolve_port, AccountMode, Config, IbConfig, PipelineConfig, StorageConfig};
...
impl Config {
    /// 加载 yaml（`CONFIG_PATH` 或 `config.yaml`），再应用环境变量覆盖。
    ...
    fn from_yaml(path: impl AsRef<Path>) -> anyhow::Result<Self> {
